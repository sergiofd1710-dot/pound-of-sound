...
## Демо-функционал (работает локально, без бэкенда)

Все «социальные» возможности из конспекта реализованы и работают **локально**:
данные хранятся в `localStorage` браузера (модуль `src/lib/demoStore.ts`), без
сервера и API. Этого достаточно для показа проекта — всё кликается и
сохраняется между перезагрузками. Есть засеянные демо-данные (лоты, темы,
отзывы, продавцы).

- **Торги** (`/auctions`) — лоты с живым обратным отсчётом, история ставок,
  ставки (для авторизованных). После вашей ставки через пару секунд её
  «перебивает» конкурент — приходит уведомление.
- **Клуб** (`/club`) — темы по категориям, создание тем и ответов.
- **Чаты** (`/chats`) — переписка с продавцом; кнопка «Написать продавцу» в
  объявлении создаёт диалог, собеседник отвечает автоответом (демо).
- **Профиль** (`/profile`) — аватар, город (редактируется), мои объявления,
  лоты и безопасные сделки.
- **Страница продавца** (`/seller/:username`) — рейтинг, объявления, лоты,
  отзывы.
- **Отзывы** — звёзды + текст, рейтинг продавца пересчитывается.
- **Вонтлист** (`/wishlist`) — добавляете желаемое → 🔔, если оно уже есть на
  площадке.
- **Безопасная сделка / эскроу** — 4 шага (заморозка → отправка → проверка
  48ч → перевод продавцу), комиссия 5%, статусы в профиле.
- **Уведомления** — колокольчик в шапке (перебитая ставка, новое сообщение,
  совпадение по вонтлисту, новая сделка).

> Это **демонстрация** на локальных данных. Когда решите делать серьёзно — эти
> сущности переедут в таблицы Supabase (схема описана в плане), а компоненты
> почти не изменятся: вся логика изолирована в `src/lib/demoStore.ts` и хуках.
> Сбросить демо-данные к исходным можно функцией `resetDemo()` из этого модуля.

Покупка реальных объявлений (`listings` из Supabase) и каталог — настоящие;
торги/клуб/чаты/отзывы/эскроу — демо-слой поверх.
...
|------|------------|--------|
| 1 | Auth + каталог + создание объявлений | ✅ готово (Supabase) |
| 2 | ИИ-оценка (демо-режим, локально) | ✅ готово |
| 3 | Торги/аукционы (таймеры, ставки, история) | ✅ готово (демо) |
| 4 | Клуб/форум (темы, посты) | ✅ готово (демо) |
| 5 | Чаты, профили, страница продавца, отзывы, вонтлист | ✅ готово (демо) |
| 6 | Эскроу / безопасная сделка | ✅ готово (демо) |
| — | Перенос демо-слоя в Supabase + реальная ИИ-оценка | при необходимости |

«Демо» = работает локально на `localStorage`, без бэкенда. «Supabase» = реальный
бэкенд. Перевод демо-фич на боевой бэкенд — следующий большой шаг, когда
проект покажет потенциал.
